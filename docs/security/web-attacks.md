...
テンプレートエンジンの構文（`{{ }}`など）を悪意ある入力として送り込み、サーバーやブラウザ上で任意のコードを実行させる攻撃。
...
AngularJS では `{{ constructor.constructor('alert(1)')() }}` のような入力でスクリプト実行につながる。XSS の亜種として扱われることが多い。

### 確認方法

`{{ 7*7 }}` を入力欄に入れて `49` が表示されれば、テンプレートとして評価されている可能性がある。
