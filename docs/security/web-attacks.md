...
テンプレートエンジンの二重波括弧構文などを悪意ある入力として送り込み、サーバーやブラウザ上で任意のコードを実行させる攻撃。
...
| ディレクトリトラバーサル | 非公開ファイルへのアクセス | パス検証・ホワイトリスト・IDマッピング |
| テンプレートインジェクション | 任意コード実行・情報漏洩 | 入力を値として渡す・サンドボックス |
