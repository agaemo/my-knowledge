...
::: danger エージェントへの Indirect Injection
外部からデータを取得・処理するエージェントで最も深刻。悪意あるコンテンツが「命令」として実行されると、ファイル削除・データ送信などの取り消し不可能な操作が行われる危険がある。
:::
...
::: warning 多層防御で被害を最小化する
LLM はテキストを「理解」するため、どんな入力フィルタも回避される可能性がある。サニタイズ・権限最小化・Human-in-the-loop を組み合わせて被害を最小化する考え方が現実的。
:::
