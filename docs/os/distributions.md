...
宣言的設定と再現可能ビルドを核とする系統。

| ディストリビューション | 特徴 |
|---|---|
| **NixOS** | OS全体をNix言語で宣言的に記述する。設定ファイル1つで環境を再現でき、世代管理によるロールバックが可能 |

パッケージ管理：`nix`  
設定ファイル：`/etc/nixos/configuration.nix`（1ファイルにパッケージ・サービス・ユーザーを記述）

他の系統との違いは、パッケージをグローバルにインストールせず `/nix/store` に隔離して管理する点。依存関係の競合が起きないため、同一パッケージの複数バージョンを共存させられる。再現性が求められる開発環境や研究用途で選ばれる。

### セキュリティ特化

ペネトレーションテスト・フォレンジック・セキュリティ研究向けに、攻撃ツール群をプリインストールしたディストリビューション。

| ディストリビューション | ベース | 特徴 |
|---|---|---|
| **Kali Linux** | Debian | 最も広く使われるペンテスト用。600以上のセキュリティツールを収録 |
| **Parrot OS** | Debian | Kaliより軽量。ペンテストに加えてプライバシー・開発用途にも対応 |
| **BlackArch** | Arch Linux | 2,800以上のセキュリティツールを持つArch系。上級者向け |

これらは通常の用途には使わない。セキュリティツールが大量にインストールされた状態は攻撃面が広く、日常利用には不向き。CTF・ラボ環境・仮想マシン上での利用が一般的。
