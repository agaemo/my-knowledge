...
─────────────────────────            ─────────────────────────
  ...
  エンジニア (L4)
          │
  シニアエンジニア (L5) ───────────→  エンジニアリングマネージャー
          │                                      │
  スタッフエンジニア (L6) ──────────→  シニアEM / グループマネージャー
  (Tech Lead / Architect             │
   / Solver / Right Hand)            │
          │                エンジニアリングディレクター
  シニアスタッフ /                    │
  プリンシパル (L7)             VP of Engineering
          │                          │
  ディスティングイッシュト /          CTO
  フェロー (L8+)
```

矢印（`→`）はトラック転換のタイミングの例。往復も一般的。
...
各レベルに期待されることを文書化したフレームワーク。「ラダー（梯子）」は段階的な昇進の比喩。

### なぜ存在するか

レベルが口頭の慣習だけで運用されると、昇進の判断が評価者の主観に依存し、公平性・透明性が失われる。ラダーを整備することで：

- エンジニアが「次のレベルに上がるために何をすればよいか」を自分で把握できる
- マネージャーが昇進推薦をする際に客観的な根拠として使える
- 評価・報酬の一貫性をチーム・部門間で保てる

### 何が書かれているか

典型的なラダーは各レベルに対して以下の軸で期待値を定義する。

| 軸 | 内容の例 |
|---|---|
| **テクニカルスキル** | 設計・コード品質・システム理解の深さ |
| **スコープ** | タスク・機能・チーム・組織のどこまでを自律的に扱えるか |
| **影響力** | 自分・チーム・複数チーム・会社のどこに貢献しているか |
| **コラボレーション** | 他者との働き方・メンタリング・フィードバックの質 |
| **曖昧さへの対処** | どれだけ不明確な問題を自力で前進させられるか |

### 公開ラダーの例

多くの企業がラダーを社外公開しており、自社のラダー整備の参考にされることが多い。Dropbox・Buffer・CircleCI・Rent the Runway などが有名。

### 注意点

ラダーは「チェックリスト」ではない。全項目を満たしたら自動昇進するものではなく、あくまで期待値の目安。昇進の判断には「インパクトの実績」と「次のレベルで活躍できる準備ができているか」が重視される。
