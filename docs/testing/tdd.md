...
## Bug-fix TDD（リグレッション駆動）

バグが報告されたとき、**修正の前にそのバグを再現するテストを書く**アプローチ。

```
1. Red    — バグを再現する失敗テストを書く
2. Green  — テストが通るよう修正する
3. Refactor — 必要であれば整理する
```

通常の TDD と同じサイクルだが、起点が「新機能の仕様」ではなく「バグ報告」になる。

### なぜテストを先に書くか

- **再現確認**: テストが失敗することで、バグを正しく再現できていることを確認できる
- **修正確認**: テストが通ることで、修正が正確であることを確認できる
- **リグレッション防止**: 同じバグの再発を自動的に検出できる

```ts
// バグ報告：「数量が 0 のとき合計金額が null になる」
// 修正前にテストを書く
test('数量が 0 のとき合計金額は 0 になる', () => {
  const order = new Order([{ price: 100, quantity: 0 }]);
  expect(order.total()).toBe(0); // ← このテストが Red になることを確認してから修正する
});
```

バグ修正の PR には必ずこのテストを含める。テストがない修正はリグレッションのリスクを残す。
