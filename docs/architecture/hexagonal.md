...
```mermaid
flowchart LR
    subgraph driving["Driving 側　外 → 内"]
        direction TB
        HTTP["HTTP"]
        CLI["CLI"]
        Test["Test"]
    ...
    subgraph hex["Application (Hexagon)"]
        DP["Driving Port"] --> UC["UseCase"] --> DrP["Driven Port"]
    end

    subgraph driven["Driven 側　内 → 外"]
        direction TB
        PG["PostgreSQL"]
        ExtAPI["外部API"]
        Mail["メール"]
    ...
    style UC fill:#fef9c3,stroke:#d97706
    style DP fill:#dbeafe,stroke:#2563eb
    style DrP fill:#dcfce7,stroke:#16a34a
```
